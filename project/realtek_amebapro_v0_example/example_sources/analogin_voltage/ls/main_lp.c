...
#define MBED_ADC_EXAMPLE_PIN_1    PA_2	
#define MBED_ADC_EXAMPLE_PIN_2    PA_3	
#define MBED_ADC_EXAMPLE_PIN_3    PA_4	
...
)
{
	...
	dbg_printf("ADC:offset = 0x%x, gain = 0x%x\n\r", offset, gain);
	...
	for (;;){
		...
		dbg_printf("AD0:%x = %d mv, AD1:%x = %d mv, AD2:%x = %d mv\n\r", adcdat0, v_mv0, adcdat1, v_mv1, adcdat2, v_mv2); 
		adc_delay();
	}
