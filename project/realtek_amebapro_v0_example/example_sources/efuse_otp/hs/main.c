...
static void efuse_otp_task(void *param)
{
	...
	if(ret < 0){
		dbg_printf("efuse OTP block: read address and length error\n");
		goto exit;
	}
	...
	if(0){ // write
		...
		device_mutex_unlock(RT_DEV_LOCK_EFUSE);
		if(ret < 0){
			dbg_printf("efuse OTP block: write address and length error\n");
			goto exit;
		}
		dbg_printf("\nWrite Done.\n");
	}
	...
	dbg_printf("efuse OTP block: Test Done\n");
	vTaskDelete(NULL);
	
exit:
	dbg_printf("efuse OTP block: Test Fail!\n");
	vTaskDelete(NULL);
}
