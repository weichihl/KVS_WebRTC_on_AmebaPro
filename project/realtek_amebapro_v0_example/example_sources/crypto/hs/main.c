...
int test_md5(void)
{
    ...
    if (SUCCESS != ret) {
        printf("crypto_md5 failed,ret = %d\r\n", ret);
        goto test_md5_end;
    }
	if (memcmp(digest, md5_digest, 16) == 0) {
        printf("MD5 test result is correct\r\n");
        ret = SUCCESS;
	} else {
		//DiagPrintf("MD5 test result is WRONG!!, ret=%d\r\n", ret);
        printf("MD5 test result is WRONG!!\r\n");
	...
	for (i = 0; i < 14; i++) {	
		...
		if (memcmp(md5sum, md5_test_sum[i], 16) != 0) {
            printf("MD5 test result is WRONG!!, ret=%d\r\n");
...
int test_sha2_256(void)
{
    ...
    for (i = 0 ; i < 2; i++) {
        ...
        if (AUTH_SHA2_256 == i){
            ...
            if (SUCCESS != ret) {
                printf("crypto_sha2_256 failed,ret = %d\r\n", ret);
                goto test_sha2_end;
            }
        }else if (AUTH_HMAC_SHA2_256 == i) {
            printf("SHA2 HMAC test\r\n");
            ret = crypto_hmac_sha2_256(plaintext, strlen((const char *)plaintext), sha2_256_hmac_key,
                                       strlen((const char*)sha2_256_hmac_key), digest); // the length of SHA2_256's digest is 32 bytes. 
            if (SUCCESS != ret) {
                printf("crypto_hmac_sha2_256 failed,ret = %d\r\n", ret);
                goto test_sha2_end;
            }
        }
        
    	if (memcmp(digest, &sha2_256_digest[i][0], 32) == 0) {
            printf("SHA2_256 test result is correct\r\n");
            ret = SUCCESS;
    	} else {
            printf("SHA2_256 test result is WRONG!!\r\n");
    	...
    	for (j = 0; j < 14; j++) {
            if (0 == i){
                ...
                printf( " HMAC SHA2 test #%d: ", j + 1);
                ret = crypto_hmac_sha2_256(auth_plaintext_test_buf[j], auth_plaintext_test_buflen[j], sha2_256_hmac_key,
                                           strlen((const char*)sha2_256_hmac_key), sha2_256sum); // the length of SHA2_256's digest is 32 bytes. 
                if (SUCCESS != ret) {
                    printf("crypto_hmac_sha2_256 failed,ret = %d\r\n", ret);
                    break;
                }
    		...
    		if (memcmp(sha2_256sum, &sha2_256_test_sum[i][j][0], 32) != 0) {
                printf("SHA2_256 test result is WRONG!!, ret=%d\r\n");
...
int test_aes_256(void)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (CIPHER_AES_256_ECB == i) {
            printf("AES 256 ECB test Encrypt\r\n");
            pIv = NULL;
	        ivlen = 0;
            ret = crypto_aes_ecb_init(key,keylen);
        	if (SUCCESS != ret) {
                printf("AES ECB init failed, ret = %d\r\n",ret);
        		goto test_aes_end;
        	}
        	ret = crypto_aes_ecb_encrypt(message, msglen, pIv, ivlen, pResult);
        	if (SUCCESS != ret) {
                printf("AES ECB encrypt failed, ret = %d\r\n",ret);
        		goto test_aes_end;
        	}
        } else if (CIPHER_AES_256_CBC == i) {
            printf("AES 256 CBC test Encrypt\r\n");
            ret = crypto_aes_cbc_init(key,keylen);
        	if (SUCCESS != ret) {
                printf("AES CBC init failed, ret = %d\r\n",ret);
        		goto test_aes_end;
        	}
        	ret = crypto_aes_cbc_encrypt(message, msglen, pIv, ivlen, pResult);
        	if (SUCCESS != ret) {
                printf("AES CBC encrypt failed, ret = %d\r\n",ret);
        		goto test_aes_end;
        	}
        }
        if (memcmp(&aes_test_res[i][0], pResult, msglen) == 0) {
    		printf("AES 256 encrypt result success\r\n");
            ret = SUCCESS;
    	} else {
    		printf("AES 256 encrypt result failed\r\n");
        ...
        if (CIPHER_AES_256_ECB == i) {
            printf("AES 256 ECB test Decrypt\r\n");
            ret = crypto_aes_ecb_decrypt(message, msglen, pIv, ivlen, pResult);
        	if (SUCCESS != ret) {
                printf("AES ECB decrypt failed, ret = %d\r\n",ret);
        		goto test_aes_end;
        	}
        } else if (CIPHER_AES_256_CBC == i) {
            printf("AES 256 CBC test Decrypt\r\n");
        	ret = crypto_aes_cbc_decrypt(message, msglen, pIv, ivlen, pResult);
        	if (SUCCESS != ret) {
                printf("AES CBC decrypt failed, ret = %d\r\n",ret);
        		goto test_aes_end;
        	}
        }

        if (memcmp(aes_test_buf, pResult, msglen) == 0) {
            printf("AES CBC decrypt result success\r\n");
            ret = SUCCESS;
    	} else {
            printf("AES CBC decrypt result failed\r\n");
...
int test_aes_256_gcm(void)
{
    ...
    for (i = 0; i < 1; i++) {
        ...
        printf("AES 256 GCM test Encrypt\r\n");
        ret = crypto_aes_gcm_init(key,keylen);
        if (SUCCESS != ret) {
            printf("AES GCM init failed, ret = %d\r\n",ret);
            goto test_aes_gcm_end;
        }
        ret = crypto_aes_gcm_encrypt(message, msglen, nonce, pAAD, aadlen, pResult, pTag);
        if (SUCCESS != ret) {
            printf("AES GCM encrypt failed, ret = %d\r\n",ret);
            goto test_aes_gcm_end;
        }
 
        if (memcmp(aes_gcm_test_res, pResult, msglen) == 0) {
    		printf("AES GCM 256 encrypt result success\r\n");
            if (memcmp(aes_gcm_test_tag, pTag, 16) == 0) {
    		    printf("AES GCM 256 encrypt tag success\r\n");
                ret = SUCCESS;
            } else {
                printf("AES GCM 256 encrypt tag failed\r\n");
    		...
    		printf("AES GCM 256 encrypt result failed\r\n");
        ...
        printf("AES 256 GCM test Decrypt\r\n");
        ret = crypto_aes_gcm_decrypt(message, msglen, nonce, pAAD, aadlen, pResult, pTag);
        if (memcmp(aes_gcm_test_buf, pResult, msglen) == 0) {
    		printf("AES GCM 256 decrypt result success\r\n");
            if (memcmp(aes_gcm_test_tag, pTag, 16) == 0) {
    		    printf("AES GCM 256 decrypt tag success\r\n");
                ret = SUCCESS;
            } else {
                printf("AES GCM 256 decrypt tag failed\r\n");
    		...
    		printf("AES GCM 256 decrypt result failed\r\n");
...
int main(void)
{
    ...
    ret = test_md5();
    if (SUCCESS != ret) {
        printf("crypto test_md5 failed\r\n");
        goto vrf_end;
    }
    
	ret = test_sha2_256();
    if (SUCCESS != ret) {
        printf("crypto test_sha2_256 failed\r\n");
        goto vrf_end;
    }   
    ret = test_aes_256();
    if (SUCCESS != ret) {
        printf("crypto test_aes_256 failed\r\n");
        goto vrf_end;
    }
    ret = test_aes_256_gcm();
    if (SUCCESS != ret) {
        printf("crypto test_aes_256_gcm failed\r\n");
        goto vrf_end;
    }
vrf_end:
    ret = crypto_deinit();
	if (SUCCESS != ret) {
        printf("crypto engine deinit failed\r\n");
    }
